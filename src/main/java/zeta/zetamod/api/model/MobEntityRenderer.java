...
public abstract class MobEntityRenderer<T extends MobEntity, M extends EntityModel<T>> extends LivingEntityRenderer<T, M> {
    ...
    private <E extends Entity> void method_4073(T mobEntity, float f, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, E entity) {
        matrixStack.push();
        double d = (double)(MathHelper.lerp(f * 0.5F, entity.yaw, entity.prevYaw) * 0.017453292F);
        double e = (double)(MathHelper.lerp(f * 0.5F, entity.pitch, entity.prevPitch) * 0.017453292F);
        double g = Math.cos(d);
        double h = Math.sin(d);
        double i = Math.sin(e);
        ...
        double j = Math.cos(e);
        double k = MathHelper.lerp((double)f, entity.prevX, entity.getX()) - g * 0.7D - h * 0.5D * j;
        double l = MathHelper.lerp((double)f, entity.prevY + (double)entity.getStandingEyeHeight() * 0.7D, entity.getY() + (double)entity.getStandingEyeHeight() * 0.7D) - i * 0.5D - 0.25D;
        double m = MathHelper.lerp((double)f, entity.prevZ, entity.getZ()) - h * 0.7D + g * 0.5D * j;
        double n = (double)(MathHelper.lerp(f, mobEntity.bodyYaw, mobEntity.prevBodyYaw) * 0.017453292F) + 1.5707963267948966D;
        Vec3d vec3d = mobEntity.getLeashOffset();
